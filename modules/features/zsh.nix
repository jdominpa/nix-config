...
in
{
  flake.nixosModules.zsh =
    { config, ... }:
    {
      ...
      # collides with the wrapper's `bin/zsh`. hiPrio makes the wrapper win, so
      # the install module's own `enable` stays off.
      environment.systemPackages = [ (lib.hiPrio config.wrappers.zsh.wrapper) ];
  ...
  flake.darwinModules.zsh =
    { config, ... }:
    {
      ...
      environment.shells = [ config.wrappers.zsh.wrapper ];
    };
  ...
  flake.wrappers.zsh =
    ...
    in
    {
      ...
      zshrc.content = ''
        ...
        export STARSHIP_CONFIG="${starship.configuration.constructFiles."starship.toml"}"
