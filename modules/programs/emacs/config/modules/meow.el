...
(use-package meow-core
  ...
  (defun +meow-turn-on-meow-mode-h ()
    ...
    (when (display-graphic-p)
      ;; Disable `meow-esc-mode' outside terminal emacs since it isn't needed
      (meow-esc-mode -1)))
  (defun +meow--bind-keys ()
    "Define `meow-mode' keybinds."
  ...
  (setq
   ...
   ;; More ergonomic prefix locations
   meow-keypad-start-keys '((?s . ?c)
                            (?d . ?x)
                            (?h . ?h))
   ...
   meow-replace-state-name-list '((normal . "[N]")
                                  ...
                                  (insert . "[I]")
                                  (beacon . "[B]"))
   meow-char-thing-table '((?q . window)
                           (?w . defun)
                           (?e . symbol)
                           (?r . string)
                           (?t . latex)
